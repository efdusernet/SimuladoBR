...
const { badRequest, internalError, tooManyRequests } = require('../middleware/errors');
...
} = require('../utils/insightsGeminiQuota');
...
async function countGeminiFlashClicks(userId) {
  if (!db || !db.sequelize) return 0;
  try {
    const rows = await db.sequelize.query(
      `
        SELECT COUNT(*)::bigint AS n
        FROM public.user_ai_insights_click
      ...
      `,
      {
        replacements: { userId, provider: TARGET_PROVIDER, model: TARGET_MODEL },
...
async function recordGeminiFlashClick(userId) {
  if (!db || !db.sequelize) return;
  try {
    await db.sequelize.query(
      `
        INSERT INTO public.user_ai_insights_click (user_id, provider, model, used_llm, created_at)
        VALUES (:userId, :provider, :model, true, CURRENT_TIMESTAMP)
      `,
      {
        replacements: { userId, provider: TARGET_PROVIDER, model: TARGET_MODEL },
    ...
    // Best-effort: never break Insights due to logging
    logger.warn('Falha ao gravar user_ai_insights_click', {
      userId,
...
async function getInsightsDashboard(req, res, next) {
  try {
    ...
    const [summary, daily, userRow] = await Promise.all([
      ...
      db.User ? db.User.findByPk(userId, { attributes: ['DataExame', 'BloqueioAtivado', 'PremiumExpiresAt'] }) : Promise.resolve(null),
    ]);
    ...
    // Keep the AI prompt compact (avoid timeouts with large payloads)
    ...
    // Quota: limit clicks that would result in "Gerado por IA (Gemini · gemini-2.5-flash)".
    // Applies only when Gemini provider is configured AND GEMINI_MODEL=gemini-2.5-flash.
    if (isGeminiFlashConfigured()) {
      const prem = computePremiumRemainingFromUser(userRow || { BloqueioAtivado: true, PremiumExpiresAt: null });
      if (prem.isPremium) {
        const maxClicks = maxClicksForRemainingDays({ remainingDays: prem.remainingDays, lifetime: prem.lifetime });
        if (maxClicks != null) {
          const usedClicks = await countGeminiFlashClicks(userId);
          if (usedClicks >= maxClicks) {
            return next(tooManyRequests(
              `Limite atingido para Insights gerados por IA (Gemini · ${TARGET_MODEL}).`,
              'INSIGHTS_GEMINI_FLASH_CLICK_LIMIT_REACHED',
              { provider: TARGET_PROVIDER, model: TARGET_MODEL, usedClicks, maxClicks, premium: prem }
            ));
          }
        }
      }
    ...
    // Record clicks only when the result was actually generated by Gemini and matches the target model.
    try {
      const provider = normalizeProvider(llm.llmProvider);
      const model = normalizeModelName(llm.model);
      if (llm.usedLlm && provider === TARGET_PROVIDER && model === TARGET_MODEL) {
        await recordGeminiFlashClick(userId);
      }
    } catch (_) {
...
async function getGeminiFlashUsage(req, res, next) {
  try {
    const userId = req.user && req.user.id ? Number(req.user.id) : null;
    if (!userId) return next(badRequest('Usuário não identificado', 'USER_NOT_IDENTIFIED'));

    const userRow = db.User ? await db.User.findByPk(userId, { attributes: ['BloqueioAtivado', 'PremiumExpiresAt'] }) : null;
    const premium = computePremiumRemainingFromUser(userRow || { BloqueioAtivado: true, PremiumExpiresAt: null });

    const configured = isGeminiFlashConfigured();
    const maxClicks = (configured && premium.isPremium)
      ? maxClicksForRemainingDays({ remainingDays: premium.remainingDays, lifetime: premium.lifetime })
      : null;
    const usedClicks = (configured && premium.isPremium && maxClicks != null) ? await countGeminiFlashClicks(userId) : 0;
    const blocked = Boolean(configured && premium.isPremium && maxClicks != null && usedClicks >= maxClicks);
    ...
    return next(internalError('Erro interno', 'AI_GEMINI_USAGE_ERROR', err));
  }
}
