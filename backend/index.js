...
app.get('/components/sidebar.html', (req, res) => {
	...
	try {
		...
		const injection = `\n<script>(function(){\n  async function __sbIsAdmin(){\n    try {\n      const r = await fetch('/api/users/me', { method: 'GET', credentials: 'include', cache: 'no-store', headers: { 'Accept': 'application/json' } });\n      if (!r || !r.ok) return false;\n      const me = await r.json().catch(function(){ return null; });\n      return !!(me && (me.TipoUsuario === 'admin' || me.tipoUsuario === 'admin' || me.isAdmin === true));\n    } catch(_){ return false; }\n  }\n\n  // Provide/override ensureAdminAccess for sidebar click handlers.\n  window.ensureAdminAccess = async function(){\n    return await __sbIsAdmin();\n  };\n\n  // Show Admin accordion ASAP once injected.\n  setTimeout(async function(){\n    try {\n      if (!(await __sbIsAdmin())) return;\n      const el = document.getElementById('sidebarAdminAccordion');\n      if (el) el.style.display = 'block';\n    } catch(_){ }\n  }, 0);\n\n  // Prevent non-index pages (e.g., InsightsIA) from being navigated away when clicking Admin actions\n  // that historically depended on the index.html-only admin modal.
  // Capture-phase so it runs before other handlers even in stale sidebar versions.
  document.addEventListener('click', function(e){\n    try {\n      const t = e && e.target ? e.target : null;\n      if (!t || !t.closest) return;\n      const btn = t.closest('#sidebarAdminAccordion .sidebar-accordion-item[data-admin-action]');\n      if (!btn) return;\n      const action = String(btn.getAttribute('data-admin-action') || btn.dataset && btn.dataset.adminAction || '').trim().toLowerCase();\n      if (action !== 'fixture' && action !== 'reconcile') return;\n\n      // If the admin modal exists on the page, let the page handle it normally.
      if (document.getElementById('adminModal')) return;\n\n      // Otherwise, keep the current page and open index.html in a new tab with explicit intent.
      e.preventDefault();\n      e.stopPropagation();\n      const target = '/index.html?adminModalIntent=' + encodeURIComponent(action);\n      try {\n        const topWin = (window.top || window);\n        const w = topWin.open(target, '_blank', 'noopener,noreferrer');\n        if (!w) {\n          const a = document.createElement('a');\n          a.href = target;\n          a.target = '_blank';\n          a.rel = 'noopener noreferrer';\n          document.body.appendChild(a);\n          a.click();\n          a.remove();\n        }\n      } catch(_){ }\n      try { if (window.showToast) window.showToast('Abrindo Admin em nova aba...'); } catch(_){ }\n    } catch(_){ }\n  }, true);\n})();</script>\n`;
