...
| POST | `/api/exams/submit` | JWT | Body: `{ sessionId, answers: [{ questionId, optionId?, optionIds?, response? }], partial? }` | `{ sessionId, totalQuestions, totalCorrect, totalScorableQuestions?, scorePercent, details: [{ questionId, isCorrect, isPretest? }] }` | Submete respostas (parcial ou final). Calcula score excluindo pré-teste. |
...
| DELETE | `/api/admin/exams/attempts/:attemptId` | Admin | Path: `attemptId` | `{ ok, attemptId }` | Remove histórico completo da tentativa (attempt + questions + answers). |
